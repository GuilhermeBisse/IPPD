...
int main(int argc, char **argv) {
  ...
  int base_npts = total_pts / size;
  int remainder = total_pts % size;

  int local_npts = base_npts + (rank < remainder ? 1 : 0);
  int start_index = rank * base_npts + (rank < remainder ? rank : remainder);
  double *points_local = malloc(local_npts * 2 * sizeof(double));
  //[00, 01, 02, 10, 11, 12, 20, 21, 22]
  //[0, 1, 2, 3, 4, 5, 6, 7, 8]

  for (int i = 0; i < local_npts; ++i) {
    int global_idx = start_index + i;
    int yp = global_idx / p_count;  // linha
    int xp = global_idx % p_count;  // coluna
    double px = min_x + dx * ((double)xp / p_count);
    double py = min_y + dy * ((double)yp / p_count);
    int lid = i * 2;
    points_local[lid] = px;
    points_local[lid + 1] = py;
  }
  ...
  int *local_mset = malloc(local_npts * sizeof(int));
  ...
  compute_mandelbrot(points_local, local_npts, local_mset);
  ...
  int *recvcounts = NULL;
  int *displs = NULL;
  int* mset_global = NULL;

  if(rank==0){
    recvcounts = malloc(size*sizeof(int));
    displs = malloc(size*sizeof(int));
    mset_global = malloc(total_pts*sizeof(int));
    
    int base_npts = total_pts / size;
    int remainder = total_pts % size;

    for(int i=0; i<size; i++){
        recvcounts[i] = base_npts + (i < remainder ? 1 : 0);
        displs[i] = i * base_npts + (i < remainder ? i : remainder);
    }
}

MPI_Gatherv(local_mset, local_npts, MPI_INT, mset_global, recvcounts, displs, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  if (rank == 0) {
    ...
    for (int yp = 0; yp < p_count; ++yp) {
      for (int xp = 0; xp < p_count; ++xp)
        fprintf(fd, "%d ", mset_global[yp * p_count + xp]);
      fprintf(fd, "\n");
    }
    fclose(fd);
  }
